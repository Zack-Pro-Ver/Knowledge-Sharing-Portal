import type { DropdownVariant, DropdownSize, DropdownColor } from './Dropdown.types';
...
export const getOptionStyles = (disabled: boolean, selected: boolean) => {
  const baseClasses = [
    'flex',
    'items-center',
    'px-4',
  ...
  if (disabled) {
    ...
    stateClasses.push('bg-blue-100', 'text-blue-900');
  } else {
    stateClasses.push('text-gray-900', 'hover:bg-gray-100');
  }

  return [...baseClasses, ...stateClasses].join(' ');
};

export const getIconStyles = (size: DropdownSize) => {
  const iconSizes = {
    small: 'w-4 h-4',
    medium: 'w-5 h-5',
    large: 'w-6 h-6',
...
export const getArrowStyles = () => {
  return 'ml-2 h-4 w-4 transition-transform duration-200';
};
