...
export const getCardStyles = (
  ...
  const variantClasses = {
    ...
    outlined: ["border", "border-gray-300", raised ? "hover:shadow-md" : ""],
  };

  const sizeClassList = sizeClasses[size];
  const variantClassList = variantClasses[variant];

  return [...baseClasses, ...sizeClassList, ...variantClassList]
    .filter(Boolean)
    .join(" ");
};
...
export const getCardContentStyles = (size: CardSize) => {
  ...
  return ["flex-1", ...contentSizes[size]].join(" ");
};
...
export const getTitleStyles = () => {
  return ["font-semibold", "text-gray-900", "leading-tight"].join(" ");
};

export const getSubheaderStyles = () => {
  return ["text-sm", "text-gray-600", "leading-tight"].join(" ");
};
